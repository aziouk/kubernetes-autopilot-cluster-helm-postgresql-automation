...
Replace px-user-password, px-user and px-database with desired credentials. This must be ran before the autoinstaller. 
</p>
...
echo -n "px-database" | gcloud secrets version DB_NAME --data-file=-
...
```
The above could be handled by the setenv install (please request this if needed), but it would involve automation scripts containing sensitive credentials in plaintext. It is much better not to use setenv, but it is trivial to add automation to set the password. For example if the deploy scripts are on a bastion which self destroys, then it would be potentially acceptable. Perhaps if setenv had a pre-encrypted credential values added to it, then the credentials could be ported with the template, for special circumstances, but this is not recommended, as the cipher of the variable could be more unreliable than common, maintained secret stores like GSM. It is not recommended to manage this via a bucket with a helper terraform script for this reason.

The third and middle way approach is to generate random username, password, and database credentials, with specific roles at run-time, and add the automatically generated variables to the google store, and then have the automation refer to that. At the end of the helm build calls can be made to reveal the DB_PASSWORD, DB_USER and DB_NAME generated. This change would require only trivial/arbitrary changes to be achieved if desired.
