...
# This causes grieving issues - since http is not exposed by api list becomes empty and taints build breaking it
...
# This causes builds to fail, api not exposing the option any longer, deprecated?
...
# The below commented line is related to the empty list variable breaking http lb setup of cluster because http of lb is automatic and apparently not exposed in api any longer, breaking googles own reference scripts :(
# see https://github.com/terraform-google-modules/terraform-google-kubernetes-engine/issues/1716
# see also broken documentation refering to it still :( https://cloud.google.com/kubernetes-engine/docs/tutorials/stateful-workloads/postgresql#autopilot
# This is super sad because this process should be very simple to perform with a working reference template - google manifests great evil from within by not performing unit tests against their documentation references
# for terraform, autopilot, gke cluster/helm installation examples et al.

# cluster_http_load_balancing_enabled        = !local.cluster_output_http_load_balancing_enabled
# apparently the cluster pods arent resizable horizontally or the api value isnt exposed for population into the list
